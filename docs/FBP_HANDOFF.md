...
- FBP coverage (prior baseline): `experts.py` 94%, `domainrepo.py` **100%**, driver 91%, web.py
  **79%** (was 72%) after the full-surface HTTP route suite. **The `.coverage` artifact in the repo is stale (Aug 23) and must be regenerated by the operator** (`coverage erase` + `pytest --cov=agent_centric.fbp --cov-report=term`) before trusting any number; this arc's additions are individually tested + live-verified but the aggregate is not re-measured.
...
uv sync --extra dev
uv sync --extra dev
uv run pytest                  # operator runs the suite (Law 12); ~1010+ tests
uv run ruff check .            # clean
uv run mypy src                # clean, 94 source files
uv run pytest --cov=agent_centric.fbp --cov-report=term   # operator: coverage erase first (.coverage is stale)
...
- **Input resource-bounds on every entry into the verified spine — DONE (this arc).**
  Landing server (read timeout, security headers, opaque banner, readiness/liveness,
  413 payload bound), ACP (prompt-size bound), MCP (tool-args bound), component
  networks (step-count bound), orchestration plans (step-count bound). Arcs 53-59.
  The remaining honest deploy-gaps are the by-design-not-built items above (real TLS
...
- **Unpushed commits (21) — open:** the full local arc `3da8879` → `5cac3f7` (landing hardening + captures + readiness/liveness split + ACP bound + MCP bound + operator test fixes + 413 payload bound + web.py pure-helper coverage + network step-limit + orchestration step-limit + render-branch coverage + their captures) is local, not yet pushed. `origin/agent-centric-fbp` is at `eb3a9fc`. The
